	...
	if ( version_compare( $wpVer, WP_VER_WPDW_REQUIRE, '<' ) ) {
		$e->add(
			...
			sprintf(
				...
				$wpVer, WP_VER_WPDW_REQUIRE
			)
		);
	}
	if ( $e->get_error_code() ) {
		global $_wp_domain_work_error_messages;
		$_wp_domain_work_error_messages = $e->get_error_messages();
...
function error_requirement_wp_domain_work_plugin() {
	...
	$msgs = array_map( 'messages_requirement_wp_domain_work_plugin', $_wp_domain_work_error_messages );
	$msg .= implode( "\n", $msgs );
